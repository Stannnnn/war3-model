...
function bitVal(data: Uint8Array, bitCount: number, index: number): number {
  ...
  return (
    (byte >> (valsPerByte - (index % valsPerByte) - 1)) & ((1 << bitCount) - 1)
  );
}
...
export function decode(arrayBuffer: ArrayBuffer): BLPImage {
  ...
  if (
    ...
    image.content !== BLPContent.Direct
  ) {
...
export function getImageData(
  ...
  if (blp.content === BLPContent.JPEG) {
    ...
    data.set(
      uint8Data.subarray(mipmap.offset, mipmap.offset + mipmap.size),
      headerSize
    );
    ...
    let palette = new Uint8Array(blp.data, 39 * 4, 256 * 4),
      ...
      alphaData = new Uint8Array(
        blp.data,
        mipmap.offset + size,
        Math.ceil((size * blp.alphaBits) / 8)
      ),
